...
*/
function makeSmartPerson(human) {
  return {
    name: human,
    sum: (num0, num1) => num0 + num1,
    speak: () => `Hello, my name is ${human}`,
  };
}
...
function getGermanCars(inventory) {
  let maker = []
  const cars = ['Audi', 'Mercedes-Benz', 'Volkswagen', 'BMW'];
  for (let i = 0; i < inventory.length; i++) {
    if (cars.includes(inventory[i].car_make)) {
      maker.push(inventory[i])
    }
  }
  return maker;
}
...
*/
const sum = (a, b) => a + b;
const addFive = num0 => num0 + 5;
const argTimesTwo = num0 => num0 * 2;
